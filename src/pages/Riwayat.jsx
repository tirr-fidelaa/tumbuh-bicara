...
export default function Riwayat() {
  ...
  function hapus(id) { hapusRiwayat(id); setList(getRiwayat()) }
  function hapusSemua() {
    if (window.confirm('Hapus semua riwayat skrining?')) { clearRiwayat(); setList([]) }
  }
  ...
  return (
    <div className="page-wrap">
      <div className={`${styles.header} anim-fadeup`}>
        ...
        {list.length > 0 && (
          <button className={styles.clearBtn} onClick={hapusSemua}>Hapus semua</button>
        )}
      </div>

      {list.length === 0 ? (
        <div className={`${styles.empty} anim-fadeup`} style={{ animationDelay: '.07s' }}>
          <div className={styles.emptyIcon}>📋</div>
          <p>Belum ada riwayat skrining.</p>
          <p style={{ fontSize: 13, marginTop: 4, color: 'var(--muted)' }}>
            Lakukan skrining pertama untuk melihat hasilnya di sini.
          </p>
        ...
        <div className={styles.list}>
          {list.map((item, i) => {
            const b = BADGE[item.kategori] ?? BADGE.sesuai
            return (
              <div key={item.id} className={`${styles.item} anim-fadeup`}
                style={{ animationDelay: `${i * 0.05}s` }}>
                <div className={styles.itemMain}>
                  <div className={styles.itemName}>{item.nama}</div>
                  <div className={styles.itemMeta}>Usia {item.usiaLabel} · {item.tanggal} · Skor {item.persen}%</div>
                  <span className={b.cls} style={{ marginTop: 8, display: 'inline-flex' }}>
                    <span className={b.dot} />{item.label}
                  </span>
                </div>
                <button className={styles.hapusBtn} onClick={() => hapus(item.id)} aria-label="Hapus">
                  <IconTrash />
                </button>
              </div>
            )
